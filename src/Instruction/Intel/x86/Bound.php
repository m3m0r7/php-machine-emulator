...
class Bound implements InstructionInterface
{
    ...
    public function process(RuntimeInterface $runtime, array $opcodes): ExecutionStatus
    {
        ...
        if ($opSize === 32) {
            ...
            // Sign extend
            if ($lowerBound & 0x80000000) $lowerBound = $lowerBound - 0x100000000;
            if ($upperBound & 0x80000000) $upperBound = $upperBound - 0x100000000;
            if ($regValue & 0x80000000) $regValue = $regValue - 0x100000000;
        } else {
            $lowerBound = $this->readMemory16($runtime, $address);
            $upperBound = $this->readMemory16($runtime, $address + 2);
            // Sign extend
            if ($lowerBound & 0x8000) $lowerBound = $lowerBound - 0x10000;
            if ($upperBound & 0x8000) $upperBound = $upperBound - 0x10000;
            if ($regValue & 0x8000) $regValue = $regValue - 0x10000;
        }
