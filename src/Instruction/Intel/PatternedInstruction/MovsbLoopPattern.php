 ...
 * EB yy             jmp <loop>
 *
 * This frequently appears in early boot loaders as a simple memcpy when DF=0.
 ...
 *
 * IMPORTANT: `movsb` copies forward when DF=0. We only accelerate cases where
 * memmove semantics match forward copy (dest <= src, or non-overlapping).
 */
class MovsbLoopPattern extends AbstractPatternedInstruction
{
    ...
    public function tryCompile(int $ip, array $bytes): ?callable
    {
        ...
        return function (RuntimeInterface $runtime) use ($ip, $exitIp, $patternName, &$logged): PatternedInstructionResult {
            ...
            // This loop only terminates correctly when DF=0 (forward).
            ...
            $end = $ma->fetch(RegisterType::ECX)->asBytesBySize(32) & 0xFFFFFFFF;
            ...
            if ($count === 0) {
                // Flags from `cmp ecx, edi` when equal.
            ...
            // Segment limit checks use offsets.
            ...
            // Flags from final `cmp ecx, edi` when equal.
            $ma->setCarryFlag(false);
            $ma->setZeroFlag(true);
            $ma->setSignFlag(false);
            $ma->setOverflowFlag(false);
